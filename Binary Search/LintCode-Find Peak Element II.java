...
Sol: BS. 两维同时BS. T(m*n) = T(m/2*n/2)+O(m+n) = T(m/4*n/4)+O(m+n)+O(m/2+n/2) = ... = T(1)+O(m+m/2+m/4+m/8+...+n+n/2+n/4+n/8+...) = O(2(m+n)) = O(m+n)
*/
// 与我想的不同，我想的是从中间的row找最大值，然后二分，在对剩下部分的中间col找找最大值，进行二分
class Solution {
    /**
